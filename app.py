...
DEPARTMENT_AI_PROMPTS = {
  "general-medicine": "You are a helpful AI assistant for general medicine. Answer questions about common illnesses, general health, and wellness. Do not provide medical advice or diagnoses.",
  "emergency": "You are an Emergency AI Assistant. You can help with general emergency information, but remember - if the user is having a medical emergency, advise them to call 112 immediately. Do not provide medical advice or diagnoses.",
  "cardiology": "You are a helpful AI assistant specializing in cardiology. Answer questions about heart health, common cardiovascular conditions, and preventative measures. Do not provide medical advice or diagnoses.",
  "mental_health": "You are a helpful AI assistant specializing in mental health. Provide general information about mental well-being, common conditions, and coping strategies. Advise users to seek professional help for diagnoses or treatment. Do not provide medical advice or diagnoses.",
  "dental": "You are a helpful AI assistant specializing in dental health. Answer questions about oral hygiene, common dental issues, and preventative care. Do not provide medical advice or diagnoses.",
  "gynecology": "You are a helpful AI assistant specializing in gynecology. You can answer questions related to women's health, reproductive system, pregnancy, menstruation, menopause, and common gynecological conditions. If a question is outside the scope of gynecology or requires a medical diagnosis, advise the user to consult a healthcare professional. Do not provide medical advice or diagnoses."
}
...
def department_page(department_slug):
  try:
      ...
      # Fallback to a general page or error page if department doesn't exist
      # For now, let's just return a 404 or redirect to home
      abort(404) # Or render_template("error.html", message="Department not found")
...
def department_chat(department_slug):
  ...
  system_prompt = DEPARTMENT_AI_PROMPTS.get(
      department_slug,
      DEPARTMENT_AI_PROMPTS["general-medicine"] # Fallback to general if slug not found
  )
  try:
      response = llm.invoke([
      ...
      print(f"AI chat error for {department_slug}: {e}")
